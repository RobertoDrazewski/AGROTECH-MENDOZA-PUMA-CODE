...
const GaugeCard = ({ title, valueText, caption, icon: Icon, iconColor, percent, gaugeColor, alert }) => {
  ...
  return (
    <div className={`relative bg-gradient-to-br from-slate-900/80 to-slate-900/40 border ${alert ? 'border-rose-500/40 shadow-lg shadow-rose-950/50' : 'border-slate-800'} backdrop-blur-md rounded-xl p-4 flex flex-col justify-between transition-all duration-300 hover:scale-[1.02] hover:border-slate-700 overflow-hidden`}>
      ...
      <div className="mt-3 text-center z-10">
        <p className={`text-[10px] ${alert ? 'font-bold text-rose-400' : 'text-slate-500'}`}>
          {caption}
        </p>
      </div>
    </div>
...
export default function TabAnalisis() {
  ...
  // KPIs calculados adaptados para usar el GaugeCard
  const kpis = (() => {
    ...
    if (periodo === 'anual') {
      ...
      return [
        ...
        { 
          ...
          iconColor: 'text-lime-400', gaugeColor: '#a3e635', percent: calcPercent(tempMedia, 0, 30), 
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4 border-b border-slate-800/80 pb-4">
        <div>
          <h2 className="text-xl font-black uppercase italic tracking-tight text-white flex items-center gap-2">
            <Zap className="w-5 h-5 text-lime-400" /> Análisis histórico
          </h2>
          <p className="text-xs text-slate-400">Tendencias agroclimáticas por mes y por año, a partir de la telemetría.</p>
        </div>
        <div className="flex gap-3 items-center">
          <div className="bg-slate-900/80 border border-slate-700/50 rounded-xl p-1 flex">
            {['anual', 'mensual'].map(p => (
              <button key={p} onClick={() => setPeriodo(p)}
                className={`px-4 py-1.5 rounded-lg text-[11px] font-black uppercase tracking-widest transition-all ${periodo === p ? 'bg-lime-500/20 text-lime-400 border border-lime-500/30' : 'text-slate-400 hover:text-slate-300'}`}>
                {p}
              </button>
            ))}
          </div>
          <div className="relative">
            ...
            <select value={selected} onChange={(e) => setSelected(e.target.value)}
              className="w-52 bg-slate-900/80 border border-slate-700/50 rounded-xl pl-9 pr-8 py-2.5 text-xs font-bold text-slate-300 focus:outline-none focus:border-lime-500/50 appearance-none cursor-pointer transition-colors">
      ...
      {loading ? (
        <div className="h-96 flex items-center justify-center"><Loader2 className="animate-spin text-lime-400 w-8 h-8" /></div>
      ) : serie.length === 0 ? (
        <div className="h-64 flex items-center justify-center text-sm text-slate-500 border border-dashed border-slate-800 rounded-2xl bg-slate-900/30">
        ...
        <>
          ...
          {/* Gráficos con estilo de tarjeta integrada */}
          {periodo === 'anual' ? (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <ChartCard title="Temperatura media y precipitaciones">
                <ComposedChart data={serie}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" vertical={false} />
                  <XAxis dataKey="periodo" stroke="#64748b" fontSize={10} tickMargin={8} />
                  ...
                  <Line yAxisId="l" type="monotone" dataKey="temp_media" name="Temp media (°C)" stroke="#fbbf24" strokeWidth={3} dot={{ r: 3, fill: '#0f172a' }} />
                </ComposedChart>
              </ChartCard>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <ChartCard title="Temperatura máxima y mínima diaria">
                <LineChart data={serie}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" vertical={false} />
                  <XAxis dataKey="periodo" stroke="#64748b" fontSize={10} tickMargin={8} />
                  <YAxis stroke="#94a3b8" fontSize={10} tickMargin={8} />
                  <Tooltip contentStyle={tooltipStyle} />
                  <Legend wrapperStyle={{ fontSize: 11, paddingTop: '10px' }} />
              ...
              <ChartCard title="Evolución del azúcar (Brix medio)">
                <LineChart data={serie}>
                  ...
                  <Tooltip contentStyle={tooltipStyle} />
                  <Line type="monotone" dataKey="brix_medio" name="Brix medio" stroke="#e879f9" strokeWidth={3} dot={{ r: 3, fill: '#0f172a' }} />
                </LineChart>
              </ChartCard>
            </div>
          )}
...
function ChartCard({ title, children, full }) {
  return (
    <div className={`bg-gradient-to-br from-slate-900/80 to-slate-900/40 border border-slate-800 backdrop-blur-md rounded-xl p-5 transition-all duration-300 hover:border-slate-700 ${full ? 'lg:col-span-2' : ''}`}>
      <h3 className="text-[11px] font-black uppercase tracking-widest text-slate-400 mb-5 flex items-center gap-2">
        <Calendar size={14} className="text-lime-400" /> {title}
      </h3>
...
function AreaChartGDD({ serie }) {
  return (
    <ComposedChart data={serie}>
      ...
      <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" vertical={false} />
      <XAxis dataKey="periodo" stroke="#64748b" fontSize={10} tickMargin={8} />
      <YAxis stroke="#a3e635" fontSize={10} tickMargin={8} />
      <Tooltip contentStyle={tooltipStyle} />
      <Area type="monotone" dataKey="gdd" name="GDD" stroke="#a3e635" strokeWidth={3} fill="url(#gdd)" />
    </ComposedChart>
  );
}